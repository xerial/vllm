...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_release_and_recommit_for_sleep():
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
@pytest.mark.parametrize("layout", [KVCacheLayout.LBNHC, KVCacheLayout.BLNHC])
def test_committed_views_cover_exactly_the_committed_bytes(layout):
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_defragmenting_commit_leaves_one_allocation_per_segment():
